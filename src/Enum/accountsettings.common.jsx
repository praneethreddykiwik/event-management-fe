export const HEADINGS = {
    MY_ACCOUNT: "MY ACCOUNT",
    ACCOUNT_SETTINGS: "Account Settings",
    LINKED_ACCOUNTS: "Linked Accounts",
    EMAIL_PREFERENCE: "Email Preference",
    BACKUP: "Backup",
}

export const TEXTS = { 
    EMAIL_PREFERENCE: 'Choose the type of email you want to receive from Events.',
    BACKUP: 'Automatic backup means never worrying about losing your data. Upgrade now for peace of mind.',
    MODAL_TEXT: 'Invite your friends to share the wonderful experience.'
}
export const SETTINGS_LABELS = {
    BLOCK_ADULT: "Block adult contents from gallery",
    OPT_OUT_RECOMMENDATIONS: "Opt out of recommendation based on views and subscriptions",
    NO_SEARCH_INDEX: "Don’t allow search engines to index my profile",
    AUTO_UPDATE: "Allow auto update of contents",
}

export const FOOTER_CONTENT = {
    DESCRIPTION_TEXT: "Quis autem vel eum iure reprehenderit qui in ea voluptate velit esse quam nihil molestiae consequatur.",
    CONTACT_US: "Contact Us",
    CONTACT_PHONE: "(+91) 98765 01234",
    EMAIL_ID: "events@email.com",
    LOCATION: (<>2118 Thornridge Cir.
Syracuse,<br /> Connecticut<br /> 35624</>),
    ALL_RIGHTS_RESERVED: "Eventz - All rights reserved",
    PRIVACY_POLICY: "Privacy Policy",
    TERMS_CONDITIONS: "Terms and Conditions"
}
